...
void SimController::UpdateInputEvents(float dt)
{
    ...
    else if (simRUNNING(s) || simPAUSED(s))
    {
        ...
        {
            this->SetPendingPlayerActor(m_actor_manager.FetchNextVehicleOnList(m_player_actor, m_prev_player_actor));
        }
        else if (RoR::App::GetInputEngine()->getEventBoolValueBounce(EV_COMMON_ENTER_PREVIOUS_TRUCK, 0.25f))
        {
            this->SetPendingPlayerActor(m_actor_manager.FetchPreviousVehicleOnList(m_player_actor, m_prev_player_actor));
        }
