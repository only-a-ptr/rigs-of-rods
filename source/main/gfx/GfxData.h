...
inline PropAnimFlag operator|=(PropAnimFlag a, PropAnimFlag b) { return static_cast<PropAnimFlag>(a|b); }
inline PropAnimMode operator|=(PropAnimMode a, PropAnimMode b) { return static_cast<PropAnimMode>(a|b); }

struct PropAnim
{
    float animratio; //!< A coefficient for the animation, prop degree if used with mode: rotation and propoffset if used with mode: offset.
    PropAnimFlag animFlags;
    PropAnimMode animMode;
    float animOpt3; //!< Various purposes
    float animOpt5;
    int animKey;
    int animKeyState;
    int lastanimKS;
...
struct Prop
{
    ...
    // Special props
    MeshObject*       pp_wheel_mesh_obj    = nullptr;             //!< Special prop: steering wheel
    Ogre::Vector3     pp_wheel_pos         = Ogre::Vector3::ZERO; //!< Special prop: steering wheel
    Ogre::SceneNode * pp_wheel_scene_node  = nullptr;             //!< Special prop: steering wheel
    float             pp_wheel_rot_degree  = 0;                   //!< Special prop: steering wheel
