...
void CameraManager::UpdateCurrentBehavior()
{
    ...
    case CAMERA_BEHAVIOR_STATIC:
        m_staticcam_fov_exponent = App::gfx_static_cam_fov_exp->GetActiveVal<float>();
...
bool CameraManager::Update(float dt, Actor* player_vehicle, float sim_speed) // Called every frame
{
    ...
    if (App::sim_state->GetActiveEnum<SimState>() == RoR::SimState::PAUSED || dt == 0.0f)
    {
...
void CameraManager::ResetCurrentBehavior()
{
    ...
    case CAMERA_BEHAVIOR_STATIC:
        m_staticcam_fov_exponent = 1.0f;
        App::gfx_static_cam_fov_exp->SetActiveVal(1.0f);
        return;
    ...
    case CAMERA_BEHAVIOR_VEHICLE_CINECAM:
        ...
        m_camera->setFOVy(Degree(App::gfx_fov_internal->GetActiveVal<int>()));
        return;
...
void CameraManager::ActivateNewBehavior(CameraBehaviors new_behavior, bool reset)
{
    ...
    case CAMERA_BEHAVIOR_STATIC:
        m_staticcam_previous_fov = m_camera->getFOVy();
        break;
...
void CameraManager::DeactivateCurrentBehavior()
{
    ...
    {
        m_camera->setFOVy(m_staticcam_previous_fov);
    }
    else if (m_current_behavior == CAMERA_BEHAVIOR_VEHICLE_CINECAM)
    {
        if ( m_cct_player_actor != nullptr )
        {
            m_camera->setFOVy(Degree(App::gfx_fov_external->GetActiveVal<int>()));
...
void CameraManager::UpdateCameraBehaviorStatic()
{
    ...
    if (m_staticcam_force_update || m_staticcam_update_timer.getMilliseconds() > 1000)
    {
        ...
        float cmradius = std::max(radius, App::gfx_camera_height->GetActiveVal<float>() / 7.0f);
                ...
                intersectsTerrain(m_staticcam_position, lookAt, lookAtPrediction, interval))
        {
            ...
            float desired_offset = std::max(std::sqrt(radius) * 2.89f, App::gfx_camera_height->GetActiveVal<float>());
...
bool CameraManager::CameraBehaviorStaticMouseMoved(const OIS::MouseEvent& _arg)
{
    ...
    if (ms.buttonDown(OIS::MB_Right))
    {
        ...
        App::gfx_static_cam_fov_exp->SetActiveVal(m_staticcam_fov_exponent);
        return true;
    }
...
void CameraManager::UpdateCameraBehaviorVehicle()
{
	...
	if ( RoR::App::gfx_extcam_mode->GetActiveEnum<GfxExtCamMode>() == RoR::GfxExtCamMode::PITCHING)
	{
...
void CameraManager::CameraBehaviorVehicleSplineUpdate()
{
    ...
    if (App::gfx_extcam_mode->GetActiveEnum<GfxExtCamMode>() == GfxExtCamMode::PITCHING)
    {
        m_cam_target_pitch = -asin(dir.dotProduct(Vector3::UNIT_Y));
    }
