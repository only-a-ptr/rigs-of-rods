...
void SequentialImporter::GenerateNodesForWheel(File::Keyword generated_from, int num_rays, bool has_rigidity_node)
{
    // Old parser logic:
    ...
    if (generated_from == File::KEYWORD_FLEXBODYWHEELS || generated_from == File::KEYWORD_WHEELS2)
    {
        // Rim nodes
...
unsigned SequentialImporter::GetNodeArrayOffset(File::Keyword keyword)
{
    ...
    switch (keyword)
    {
    case File::KEYWORD_FLEXBODYWHEELS: out_offset += m_num_meshwheels2_nodes; // NO break! 
    case File::KEYWORD_MESHWHEELS2:    out_offset += m_num_meshwheels_nodes;  // NO break!
    case File::KEYWORD_MESHWHEELS:     out_offset += m_num_wheels2_nodes;     // NO break!
    case File::KEYWORD_WHEELS2:        out_offset += m_num_wheels_nodes;      // NO break!
    case File::KEYWORD_WHEELS:         out_offset += m_num_cinecam_nodes;     // NO break!
    case File::KEYWORD_CINECAM:        out_offset += m_num_named_nodes;       // NO break!
    case File::KEYWORD_NODES2:         out_offset += m_num_numbered_nodes;    // NO break!
    case File::KEYWORD_NODES:          break; // Starts at 0
    default: break;
    }
    return out_offset;
}
...
Node::Ref SequentialImporter::ResolveNodeByIndex(unsigned int index_in, unsigned int def_line_number)
{
    ...
    if (index_out != index_in)
    {
        ...
        if (node_entry.origin_keyword != File::KEYWORD_NODES && node_entry.origin_keyword != File::KEYWORD_NODES2)
        {
            msg << " (generated by: " << File::KeywordToString(node_entry.origin_keyword) 
...
void SequentialImporter::ProcessModule(std::shared_ptr<RigDef::File::Module> module)
{
    ...
    FOR_EACH (File::KEYWORD_AIRBRAKES, module->airbrakes, airbrake,
    {
    ...
    FOR_EACH (File::KEYWORD_AXLES, module->axles, axle,
    {
    ...
    FOR_EACH (File::KEYWORD_BEAMS, module->beams, beam,
    {
        RESOLVE(beam.nodes[0]);
        RESOLVE(beam.nodes[1]);
    });

    FOR_EACH (File::KEYWORD_CAMERAS, module->cameras, camera,
    {
    ...
    FOR_EACH (File::KEYWORD_CAMERARAIL, module->camera_rails, rail,
    {
        RESOLVE_VECTOR(rail.nodes);
    });

    FOR_EACH (File::KEYWORD_CINECAM, module->cinecam, cinecam,
    {
    ...
    FOR_EACH (File::KEYWORD_COLLISIONBOXES, module->collision_boxes, box,
    {
        RESOLVE_VECTOR(box.nodes);
    });

    FOR_EACH (File::KEYWORD_COMMANDS2, module->commands_2, command,
    {
        RESOLVE(command.nodes[0]);
        RESOLVE(command.nodes[1]);
    });

    FOR_EACH (File::KEYWORD_CONTACTERS, module->contacters, contacter_node,
    {
        RESOLVE(contacter_node);
    });

    FOR_EACH (File::KEYWORD_EXHAUSTS, module->exhausts, exhaust,
    {
        RESOLVE(exhaust.reference_node);
        RESOLVE(exhaust.direction_node);
    });

    RESOLVE_OPTIONAL_SECTION (File::KEYWORD_EXTCAMERA, module->ext_camera,
    {
        RESOLVE(module->ext_camera->node);
    });

    FOR_EACH (File::KEYWORD_FIXES, module->fixes, fixed_node,
    {
        RESOLVE(fixed_node);
    });

    FOR_EACH (File::KEYWORD_FLARES2, module->flares_2, flare2,
    {
    ...
    FOR_EACH (File::KEYWORD_FLEXBODIES, module->flexbodies, flexbody,
    {
    ...
    FOR_EACH (File::KEYWORD_FLEXBODYWHEELS, module->flex_body_wheels, flexbodywheel,
    {
    ...
    FOR_EACH (File::KEYWORD_HYDROS, module->hydros, hydro,
    {
        RESOLVE(hydro.nodes[0]);
        RESOLVE(hydro.nodes[1]);
    });

    FOR_EACH (File::KEYWORD_MESHWHEELS, module->mesh_wheels, meshwheel, // Also `meshwheels2`
    {
    ...
    FOR_EACH (File::KEYWORD_PROPS, module->props, prop,
    {
    ...
    FOR_EACH (File::KEYWORD_RAILGROUPS, module->railgroups, railgroup,
    {
        ResolveNodeRanges(railgroup.node_list);
    });

    FOR_EACH (File::KEYWORD_ROPABLES, module->ropables, ropable,
    {
        RESOLVE(ropable.node);
    });

    FOR_EACH (File::KEYWORD_ROPES, module->ropes, rope,
    {
        RESOLVE(rope.root_node);
        RESOLVE(rope.end_node );
    });

    FOR_EACH (File::KEYWORD_ROTATORS, module->rotators, rotator,
    {
    ...
    FOR_EACH (File::KEYWORD_ROTATORS2, module->rotators_2, rotator2,
    {
    ...
    FOR_EACH (File::KEYWORD_SHOCKS, module->shocks, shock,
    {
        RESOLVE(shock.nodes[0]);
        RESOLVE(shock.nodes[1]);
    });

    FOR_EACH (File::KEYWORD_SHOCKS2, module->shocks_2, shock2,
    {
    ...
    FOR_EACH (File::KEYWORD_SOUNDSOURCES, module->soundsources, soundsource,
    {
    ...
    FOR_EACH (File::KEYWORD_TRIGGERS, module->triggers, trigger,
    {
    ...
    FOR_EACH (File::KEYWORD_TURBOPROPS2, module->turboprops_2, turboprop2,
    {
    ...
    FOR_EACH (File::KEYWORD_WHEELS, module->wheels, wheel,
    {
...
bool SequentialImporter::AddNumberedNode(unsigned int number)
{
    ...
    m_all_nodes.push_back( NodeMapEntry(File::KEYWORD_NODES, Node::Id(number), m_num_numbered_nodes) );
...
{
    auto node_entry = NodeMapEntry(File::KEYWORD_NODES2, Node::Id(name), m_num_named_nodes);
...
void SequentialImporter::AddGeneratedNode(File::Keyword generated_from, NodeMapEntry::OriginDetail detail /* = NodeMapEntry::DETAIL_UNDEFINED*/ )
{
    ...
    {
    case File::KEYWORD_CINECAM:        node_sub_index = m_num_cinecam_nodes       ++; break;
    case File::KEYWORD_WHEELS:         node_sub_index = m_num_wheels_nodes        ++; break;
    case File::KEYWORD_WHEELS2:        node_sub_index = m_num_wheels2_nodes       ++; break;
    case File::KEYWORD_MESHWHEELS:     node_sub_index = m_num_meshwheels_nodes    ++; break;
    case File::KEYWORD_MESHWHEELS2:    node_sub_index = m_num_meshwheels2_nodes   ++; break;
    case File::KEYWORD_FLEXBODYWHEELS: node_sub_index = m_num_flexbodywheels_nodes++; break;
    default: break;
    }
...
std::string SequentialImporter::GetNodeStatistics()
{
    ...
    msg << "~~~ Node statistics: ~~~"
        ...
        STAT_LINE(msg, "         nodes: ", m_num_numbered_nodes      , File::KEYWORD_NODES         );
        STAT_LINE(msg, "        nodes2: ", m_num_named_nodes         , File::KEYWORD_NODES2        );
        STAT_LINE(msg, "       cinecam: ", m_num_cinecam_nodes       , File::KEYWORD_CINECAM       );
        STAT_LINE(msg, "        wheels: ", m_num_wheels_nodes        , File::KEYWORD_WHEELS        );
        STAT_LINE(msg, "       wheels2: ", m_num_wheels2_nodes       , File::KEYWORD_WHEELS2       );
...
std::string SequentialImporter::IterateAndPrintAllNodes()
{
    ...
    for (; itor != end; ++itor)
    {
        ...
        report << "\n\t" << std::setw(3) << index << ": " << entry.node_id.ToString() 
            << " (from=" << File::KeywordToString(entry.origin_keyword)
