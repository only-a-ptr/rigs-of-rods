...
/* 
    ...
    1. There is a node array, pre-allocated to MAX_NODES items (see file BeamConstants.h).
    ...
    1. Nodes could be adressed before they were defined
       This could be detected, but for compatibility reasons, parser emitted a warning only in some cases.
       See (https://github.com/only-a-ptr/ror-legacy-svn-trunk/blob/master/source/main/physics/input_output/SerializedRig.cpp#L6755-L6761)
    ...
    3. Very hard to determine index of some nodes.
       Creating rig was traditionally a manual process, and not every section supported named nodes (i.e. axles or anything using ranges)
    ...
    Order (section name[number of nodes generated per line]):
        ...
        8. meshwheels3[num_rays*2] // New in version 0.4.8
        9. flexbodywheels[num_rays*4]
