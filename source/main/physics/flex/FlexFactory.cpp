    ...
    std::string const & tire_material_name)
{
    ...
    FlexMeshWheel* flex_mesh_wheel = new FlexMeshWheel(
        rim_prop_entity, m_rig_spawner, axis_node_1_index, axis_node_2_index, nstart, nrays,
...
void FlexBodyFileIO::OpenFile(const char* fopen_mode)
{
    ...
    char path[500];
    sprintf(path, "%s%cflexbodies_mod_%00d.dat", RoR::App::GetSysCacheDir().c_str(), PATH_SLASH, m_cache_entry_number);
