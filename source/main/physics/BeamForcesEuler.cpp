...
void Actor::CalcHydros()
{
    ...
    if (ar_hydro_dir_state != 0 || ar_hydro_dir_command != 0)
    {
        if (!ar_hydro_speed_coupling)
        {
            ...
            float smoothing   = Math::Clamp(App::io_analog_smoothing->GetActiveVal<float>(),   0.5f, 2.0f);
            float sensitivity = Math::Clamp(App::io_analog_sensitivity->GetActiveVal<float>(), 0.5f, 2.0f);
